#+----------------------------------------------+#
#| MATTEO-(V1.11 Umore,Traduttore)-ALESSANDRINI |#
#+----------------------------------------------+#
    ...
    try:
        while True:
            ...
            #-UMORE-#
                ...
                if sentiment !=0:
                    print("la tua felicità ora è: ",int(sentiment * 100),"%")
                if sentiment >= 0.2:
                    engine.say("ciao matteo,oggi sei particolarmente felice mi fa piacere che sei felice")
                    engine.runAndWait()
                if sentiment <= -0.3:
                        engine.say("ciao matteo,oggi sei particolarmente triste mi dispiace che sei triste")
                        engine.runAndWait()
            ...
            #-Calcolatrice-#  
                    if "calcola" in testo or "quanto fa" in testo: 
                        ...
                        with Microphone() as source:
                            ...
                            engine.runAndWait()  
                    #-TRADUTTORE-#
                    if "si dice" in testo or "traduci" in testo:
                        testo=testo.replace("jarvis come si dice","")  
                        if "inglese" in testo:
                            testo=testo.replace("in inglese","")
                            translator=Translator(from_lang='it',to_lang='en')
                            lingua_tradotta=translator.translate(testo)
                            print("testo in inglese si dice: ",lingua_tradotta)
                            engine.say(lingua_tradotta)   
                            engine.runAndWait() 
                        if "spagnolo" in testo:
                            testo=testo.replace("in spagnolo","")
                            translator=Translator(from_lang='it',to_lang='es')
                            lingua_tradotta=translator.translate(testo)
                            print("testo in inglese si dice: ",lingua_tradotta)
                            engine.say(lingua_tradotta)   
                            engine.runAndWait() 
                        if "francese" in testo:
                            testo=testo.replace("in francese","")
                            translator=Translator(from_lang='it',to_lang='fr')
                            lingua_tradotta=translator.translate(testo)
                            print("testo in francese si dice: ",lingua_tradotta)
                            engine.say(lingua_tradotta)   
                            engine.runAndWait() 
                        if "tedesco" in testo:
                            testo=testo.replace("in tedesco","")
                            translator=Translator(from_lang='it',to_lang='de')
                            lingua_tradotta=translator.translate(testo)
                            print("testo in tedesco si dice: ",lingua_tradotta)
                            engine.say(lingua_tradotta)   
                            engine.runAndWait() 
            #---STOP---#       
